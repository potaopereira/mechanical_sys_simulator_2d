...
SlidingBallOnSlopeSymb::SlidingBallOnSlopeSymb(

):
SlidingBallOnSlopeSymbImpl(
            ...
            {
                {
                    ...
                    .boundary_parameters = std::vector<Symbolic>({0, 0, Symbolic("radius1"), Symbolic("radius2")}),
                    .sliding = true
                }
            }
        }
...
IMRB2DPARAM()
{
    ...
    mParamList["radius2"] = std::make_tuple(
        &radius2, 1, 1000, "parameter2", "radius 2 of ellipse in mm"
    );
}